...
const { validateValueTypes, validateValues } = require('./validate')
...
const parseArgumentObject = (argRef, allObjects, parserOptions) => {
  ...
  if (Object.keys(argsMissing).length && !callbacks.hasPendingSystemCallbacks) {
    throw new ParseError({
      ...
      argsMissing: Object.values(argsMissing)
    })
  }
  ...
  while (passedArguments.length) {
    ...
    if (type.properties.takesValues) {
      ...
      if (!satisfiesNargs(values.length, valuesAmount)) {
        throw new ParseError({
          code: 'INCORRECT_NUMBER_OF_VALUES',
          argInput,
          argRef,
          activeCmd: activeCmd.ref
        })
      ...
      const validatedValueTypes = validateValueTypes(values, type)
      if (!validatedValueTypes.isValid) {
        throw new ParseError({
          code: 'INVALID_VALUE_TYPE',
          argInput,
          argRef,
          invalidValues: validatedValueTypes.invalidValues,
          activeCmd: activeCmd.ref
        })
      }

      // Check if each value satisfies any given value child objects the argument might have.
      const validatedValueOpts = validateValues(validatedValueTypes.valueData, argRef)
      if (!validatedValueOpts.isValid) {
        throw new ParseError({
          code: 'INVALID_VALUE_OPTS',
          argInput,
          argRef,
          invalidValues: validatedValueOpts.invalidValues,
          activeCmd: activeCmd.ref,
          acceptedValues: validatedValueOpts.acceptedValues
        })
      }

      // If the user's 'nargs' option is null, save the single value rather than the array.
      state[argRef.id] = takesSingleValue ? validatedValueTypes.values[0] : validatedValueTypes.values
    }
  }
