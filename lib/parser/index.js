  ...
  while (passedArguments.length) {
    ...
    if (type.properties.takesValues) {
      ...
      if (!satisfiesNargs(values.length, valuesAmount)) {
        throw new ParseError({ code: 'INCORRECT_NUMBER_OF_VALUES', argInput, argRef })
      }
      ...
      if (!validatedValues.isValid) {
        throw new ParseError({ code: 'INVALID_VALUE_TYPE', argInput, argRef, invalidValues: validatedValues.invalidValues })
      }
