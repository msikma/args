...
class ArgsArgument {
  ...
  getAllValues() {
    return this.objects.filter(obj => obj.type === 'value')
  }

  getAllAcceptedValues() {
    const argValues = this.getAllValues()
    return argValues.map(argValue => argValue.content)
  }

  validateValue(value) {
    const argValues = this.getAllValues()
    
    // Always validate true if there are no values on this argument.
    if (argValues.length === 0) {
      return true
    }

    // Check all argument values to see if our passed value has been defined on them.
    for (const argValue of argValues) {
      if (argValue.content === value) {
        return true
      }
    }
