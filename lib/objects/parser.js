...
const { processAssets, prefixArgumentCodes } = require('../lang/process')
...
class ArgsParser {
    ...
    if (exitOnFeedback) {
      ...
      for (const callback of systemCallbacks) {
        ...
        if (callback.identifier === 'usage') {
          ...
          const version = result ?? this.formatVersionString()
  ...
  /**
   * Adds system arguments, such as the 'usage' and 'version' arguments.
   * 
   * This is designed to help simplify the interface. Instead of requiring the user
   * to add their own --help or --version arguments, we do it for them automatically.
   * 
   * Adds --version if addVersion is true.
   * Adds --help (usage) if addHelp is true. Both of these are true by default.
   * 
   * All system arguments are added at a lower priority than default (1000)
   * to ensure they are displayed at the bottom of the list.
   */
  addSystemArguments() {
    const { parserOptions } = this.settings
    const L = this.settings.langAssets

    if (parserOptions.addVersion) {
      this.addArgument(
        prefixArgumentCodes(parserOptions.useSingleDashOptions, L.argumentVersion),
        { description: L.argumentVersionDesc, type: 'version', priority: 100 }
      )
    }
    if (parserOptions.addHelp) {
      this.addArgument(
        prefixArgumentCodes(parserOptions.useSingleDashOptions, L.argumentHelp),
        { description: L.argumentHelpDesc, type: 'usage', priority: 200 }
      )
    }
  }
  ...
  formatVersionString() {
    return this.formatter.formatVersionString(this)
  }
