...
class ArgsCommand {
  ...
  getAllObjectsOfType(type, maxDepth = 0, currentDepth = 0) {
    ...
    // Sort objects by priority before returning.
    command.objects = this.getSortedObjects(command.objects)
  ...
  /**
   * Returns an object's arguments after sorting by priority.
   * 
   * If two arguments have the same priority (normally their priority is always 1000),
   * the _id is used instead, meaning they are returned in the order they were defined.
   */
  getSortedObjects(objectArray) {
    return orderBy(objectArray, ['opts.priority', 'id'], ['desc', 'asc'])
  }
  ...
  getSectionTree(section) {
    const children = section.objects.filter(obj => ['argument', 'command'].includes(obj.type))
    return {
      title: section.title,
      type: 'arguments',
      object: section,
      children: this.getSortedObjects(children).map(obj => obj.id)
    }
  }
